...
```
#### 3. 交互窗口设计
&emsp;&emsp;在run.py中，我们设计了一个简单的图形界面，作为程序的交互窗口。窗口界面主要包括玩家身份选择、控制游戏开始、输入和输出显示三个功能。具体实现方法如下：
- **玩家身份选择**
-利用tkinter中的Comobox控件，创建各个玩家的身份下拉菜单。在游戏开始后，将用户的选择传入Config中的角色身份字典role_identity中。
- **控制游戏开始**
-按黑夜-白天-黑夜-……的逻辑定义run_game()游戏运行函数，并在每一个日夜结束后判断游戏是否结束。利用tkinter中的Button控件，创建“开始游戏”和“发送”按钮。将开始游戏按钮的回调函数与run_game()函数进行绑定，实现点击按钮启动游戏流程。
- **输入和输出显示**
- 
